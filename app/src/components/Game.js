...
export default function Game({ setScorePrompt }) {
  useEffect(() => {
    initialize();
  }, []);
  ...
  function initialize() {
    ...
    fetch("https://api.npoint.io/0231f4e34c7c361db318")
    ...
    function Platform() {
      ...
      if (score >= 10000) this.types = [3, 3, 3, 4, 4, 4, 4];
      else if (score >= 5000) this.types = [2, 3, 3, 3, 4, 4, 4, 4];
    ...
    function init() {
      ...
      let scoreIndex = 1;
      function rocketCalc() {
        /*if (score === 400 * scoreIndex + 180) {
          rocketRender = false;
          scoreIndex = scoreIndex + 2;
        }
        if ((score === 400 * scoreIndex - 20 || rocketRender) && !rocketing) {
        rocketRender = true;*/
        var s = Rocket;
        var p = platforms[0];
      ...
      function collides() {
        ...
        if (
          player.vy > 0 &&
          player.x + 15 < r.x + r.width &&
          player.x + player.width - 15 > r.x &&
  ...
  return (
    <div id="gameContainer">
      <div id="mainMenu">
        <h1 style={{ fontSize: "58px" }}>doodle jumper</h1>
        <h3>DIscord ID required for whitelist spot</h3>
        <input
          id="userNameGame"
          type="text"
          onChange={(e) => {
      ...
      <img id="sprite" alt="sprite" src="https://i.imgur.com/jVXqI8A.png" />
